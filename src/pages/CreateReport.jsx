import { Container, Title, Card, Text, Group, Button, Stepper, Slider, Textarea, Select, Divider, Stack, Badge } from '@mantine/core';
...
import { createPerformanceReport, EVALUATION_CATEGORIES, getAllQuestions } from '../types';
...
const CreateReport = () => {
  ...
  const form = useForm({
    initialValues: {
      month: new Date().toLocaleDateString("en-CA", { year: "numeric", month: "2-digit" }).slice(0, 7), // YYYY-MM format
    ...
    validate: {
      ...
      highlights: (value) => (value.length < 10 ? 'Destaques devem ter pelo menos 10 caracteres' : null),
      pointsToDevelop: (value) => (value.length < 10 ? 'Pontos a desenvolver devem ter pelo menos 10 caracteres' : null),
    },
  });
  ...
  const nextStep = () => {
    if (active === 2) {
      // Validate required fields before going to final step
  ...
  // Calculate preview scores for final step
  ...
  return (
    <Container size="lg">
      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder>
        <Title order={2} mb="xs">Nova Avaliação de Performance</Title>
        <Text c="dimmed" mb="xl">
        ...
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stepper active={active} onStepClick={setActive} breakpoint="sm" mb="xl">
                      ...
                      {category.questions.map((question) => (
                        <div key={question.key}>
                          <Group justify="space-between" mb="xs">
                            <div>
                              <Text fw={500} size="sm">{question.label}</Text>
                              <Text size="xs" c="dimmed">
                                Peso na categoria: {question.weight}
                              </Text>
                            </div>
                            <Group gap="xs">
                              <Text size="sm" c={getSliderColor(form.values[question.key])}>
            ...
            <Stepper.Step label="Comentários" description="Feedback qualitativo">
              <Textarea
            ...
            <Stepper.Completed>
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                ...
                <Title order={3} mt="md" mb="xs">Avaliação Concluída</Title>
                <Text c="dimmed" mb="lg">
                  Revise as informações e clique em "Salvar Relatório" para finalizar.
                </Text>
                
                <Card withBorder p="md" mb="lg">
                  <Title order={4} mb="md">Resumo da Avaliação</Title>
                  
                  <Stack gap="sm" mb="md">
                    {Object.entries(EVALUATION_CATEGORIES).map(([categoryKey, category]) => {
                      const previewScores = calculatePreviewScores();
                      const categoryScore = previewScores.categoryScores[categoryKey];
                  ...
                  <Group justify="space-between">
                    ...
                    <Text fw={700} c="blue" size="lg">
                      {calculatePreviewScores().weightedAverageScore.toFixed(1)}/10
                    </Text>
                  </Group>
                </Card>
              </div>
